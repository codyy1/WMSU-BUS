...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['request_reset'])) {
    ...
    if (!$email) {
        ...
        // Find user by email or WMSUID (Student or Staff only)
        $stmt = $conn->prepare("SELECT UserID, Email, WMSUID FROM Users WHERE (Email = ? OR WMSUID = ?) AND UserType IN ('Student', 'Staff') LIMIT 1");
        ...
        if ($res && $res->num_rows > 0) {
            ...
            if ($ins->execute()) {
                ...
                $reset_link = "http://localhost/WMSUBUS/user/reset_password.php?token=" . urlencode($token);
                $reset_token = $token;
                $show_token = true;
            ...
            $message = "No account found with that email or WMSU ID.";
        }
        $stmt->close();
    }
}
...
<head>
    ...
    <style>
        ...
        input[type="text"]:focus, input[type="email"]:focus { outline: none; border-color: #2563eb; box-shadow: 0 0 0 3px rgba(37, 99, 235, 0.1); }
        .button { width: 100%; padding: 10px; background: #2563eb; color: #fff; border: none; border-radius: 8px; font-weight: 600; cursor: pointer; font-size: 14px; }
        ...
        .token-box { background: #f3f4f6; padding: 12px; border-radius: 8px; margin: 12px 0; border-left: 4px solid #2563eb; }
        ...
        .link-box { background: #eff6ff; padding: 12px; border-radius: 8px; margin: 12px 0; border-left: 4px solid #2563eb; }
...
<div class="container">
    <div class="card">
        ...
        <div class="back-link">
            <a href="index_user.php">← Back to Login</a>
        </div>
    </div>
</div>
</body>
